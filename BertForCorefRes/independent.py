...
class CorefModel(object):
  ...
  def tensorize_example(self, example, is_training):
    ...
    for cluster_id, cluster in enumerate(clusters):
      for mention in cluster:
        cluster_ids[gold_mention_map[tuple(mention)]] = cluster_id + 1 # mi -> ci
  ...
  def load_eval_data(self, is_debugging):
    if self.eval_data is None or is_debugging:
      def load_line(line):
  ...
  def evaluate(self, session, global_step=None, official_stdout=False, keys=None, eval_mode=False, is_debugging=False, save_results_to_file=False):
    self.load_eval_data(is_debugging)
    ...
    doc_keys = []
    num_evaluated= 0
    domain_to_evaluator = defaultdict(lambda: metrics.CorefEvaluator())
    ...
    for example_num, (tensorized_example, example) in enumerate(self.eval_data):
      ...
      domain = example['doc_key'].split('_')[0]
      self.evaluate_coref(top_span_starts, top_span_ends, predicted_antecedents, example["clusters"], domain_to_evaluator[domain])
      # if example_num % 10 == 0:
    ...
    if save_results_to_file:
      with open(os.environ['eval_results_fp'], 'w') as file:
        file.write('domain,metric,p,r,f1\n')
        # save precision, recall, f1 for each domain
        for domain, evaluator in sorted(domain_to_evaluator.items(), key=lambda x: x[0]):
          for c_metric in evaluator.evaluators:
            file.write('{},{},{:.2f},{:.2f},{:.2f}\n'.format(domain, c_metric.metric.__name__, c_metric.get_precision() * 100, c_metric.get_recall() * 100, c_metric.get_f1() * 100))
        # save the average precision, recall, f1 for all domains
        file.write('all,avg_conll,{:.2f},{:.2f},{:.2f}\n'.format(p * 100, r * 100, f * 100))
        for c_metric in coref_evaluator.evaluators:
          file.write('all,avg_{},{:.2f},{:.2f},{:.2f}\n'.format(c_metric.metric.__name__, c_metric.get_precision() * 100, c_metric.get_recall() * 100, c_metric.get_f1() * 100))
    
    # print("Average F1 (py): {:.2f}% on {} docs".format(f * 100, len(doc_keys)))
    # print("Average precision (py): {:.2f}%".format(p * 100))
    # print("Average recall (py): {:.2f}%".format(r * 100))

    return util.make_summary(summary_dict), (p, r, f), doc_keys
